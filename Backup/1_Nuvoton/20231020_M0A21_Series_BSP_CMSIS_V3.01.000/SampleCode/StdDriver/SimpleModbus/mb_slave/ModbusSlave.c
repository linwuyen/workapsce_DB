...
{
    mbus->timestamp = mbus->timetick = GET_TIMERCNT;
...
{
    if(0 == isRxEmpty((UART_T *)mbus->sci)) {
        if(SIZE_OF_RXBUF > mbus->rw_size) {
            rstModbusTimeout(mbus);
            mbus->rw_size++;
            *mbus->pFiFo.pi = rdByte((UART_T *)mbus->sci);
    ...
    case _RECEIVE_DATA_FROM_SCI:
        ...
        if(0 < isrMbusRxData(mbus)) {
            ...
            if(mbus->popcnts<mbus->pushcnts) {
                if(0 == isTxFull((UART_T *)mbus->sci)) {
									  uint8_t u8temp = (uint8_t)mbus->TxBuf[mbus->popcnts++];
                    wrBytes((UART_T *)mbus->sci, &u8temp, 1);
