		...
		switch(evTstBlock) {
		  ...
		  case _EV_UPLOAD_DATA:
				if(ptrAddress != endAddress) {
					 ...
					 ptrAddress++;
					 if((1024) <= ptrAddress) ptrAddress -= (1024);
				}
				else {
					ptrAddress = beginAddress;
...
uint32_t u32TestTimeout = (16000000/2);
...
void loopWriteBlock(void)
{
		...
		u32Tick = getTimerCounts(((TIMER_T *) ((( uint32_t)0x40000000) + 0x50000)));

		if(u32TimeStamp > u32Tick) {
				u32TimeOut = u32Tick + 16000000 - u32TimeStamp;
		}
		else {
