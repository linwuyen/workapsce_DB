...
int16_t readHoldingRegister(SCI_MODBUS *mbus)
{
    ...
    case _SEND_DATA_OUT:
        if(mbus->info.index<mbus->info.words) {
            ...
            pushModbusByte(((u16Word>>8)&0x00FF), mbus);
            mbus->info.crc = pushModbusByte((u16Word&0x00FF), mbus);
            mbus->info.index++;
            mbus->info.rwfrom++;
        }
        else {
...
int16_t ModbusCommError(SCI_MODBUS *mbus)
{
    ...
    mbus->info.tbcrc = (ST_CRC){0xFF, 0xFF, 0};
    mbus->info.crc = 0;
    pushModbusByte(pack->Slave, mbus);
...
volatile SCI_MODBUS mbcomm = {
		...
		.sci = ((UART_T *) ((( uint32_t)0x40000000) + 0x70000)),
		.slaveid = 0x03,
		.pReg = &regMbusData,
		.initReg = initRegN,
		.getReg = readRegN,
