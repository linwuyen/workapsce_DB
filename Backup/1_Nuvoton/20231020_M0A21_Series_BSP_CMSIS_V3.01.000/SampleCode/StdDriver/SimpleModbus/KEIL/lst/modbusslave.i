...
# 13 "..\\mb_slave\\..\\common.h"

# 1 "..\\mb_slave\\..\\ctypedef.h"
...
# 12 "..\\mb_slave\\..\\ctypedef.h"
# 13 "..\\mb_slave\\..\\ctypedef.h"
...
# 16 "..\\mb_slave\\..\\common.h"




# 33 "..\\mb_slave\\..\\common.h"




# 43 "..\\mb_slave\\..\\common.h"
...
{
    if(0<mbus->rw_size) {
        mbus->timetick = getTimerCounts(((TIMER_T *) ((( uint32_t)0x40000000) + 0x50000)));

        if(mbus->timestamp > mbus->timetick) {
            mbus->timeout = mbus->timetick + 16000000 - mbus->timestamp;
        }
        else {
            mbus->timeout = mbus->timetick - mbus->timestamp;
        }

        if(mbus->timeout >= (16000000/20)) {
...
{
    mbus->timestamp = mbus->timetick = getTimerCounts(((TIMER_T *) ((( uint32_t)0x40000000) + 0x50000)));
...
int16_t initMbusConfig(SCI_MODBUS *mbus)
{
		...
		mbus->pFiFo.pe = &mbus->RxBuf[128-1];
		mbus->crcdata = (ST_CRC){0xFF, 0xFF, 0};
		mbus->crc = 0x0000;
		mbus->rw_size = 0;
		mbus->info = (REG_FUNC_INFO) { . tbcrc = (ST_CRC){0xFF, 0xFF, 0}, . crc = 0, . words = 0, . index = 0, . addr = 0, . pData = 0, . group = 0, . memrange = 0, . rwfrom = 0 };
...
int16_t exeModbusSlave(SCI_MODBUS *mbus)
{
    ...
    case _RECEIVE_DATA_FROM_SCI:
        ...
        if(0 < isrMbusRxData(mbus)) {
            ...
            {
                if((0 == mbus->crc)&&(3 < mbus->rw_size)) 
                {
                    if((0 == *mbus->pFiFo.pb)||(mbus->slaveid == *mbus->pFiFo.pb)) 
                    {
                        
                        if(mbus->slaveid == *mbus->pFiFo.pb) ;
                        else                                 ;
