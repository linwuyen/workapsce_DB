...
uint32_t APROM_TEST_END  = 0x00004000UL;         
uint32_t LDROM_TEST_SIZE = 0x00000800UL;         
...
void run_crc32_checksum()
{
    ...
    chksum = FMC_GetChkSum(0x00000000UL, 0x5000 - 0x00000000UL);

    printf("  APROM CRC32 checksum .................. [0x%08x]\n", chksum);

    chksum = FMC_GetChkSum(0x5000, APROM_TEST_END - 0x5000);

    printf("  Data Flash CRC32 checksum ............. [0x%08x]\n", chksum);

    chksum = FMC_GetChkSum(0x00100000UL, LDROM_TEST_SIZE);
...
int32_t  flash_test(uint32_t u32StartAddr, uint32_t u32EndAddr, uint32_t u32Pattern)
{
    ...
    for (u32Addr = u32StartAddr; u32Addr < u32EndAddr; u32Addr += (0x200))
    {
        ...
        if (verify_data(u32Addr, u32Addr + (0x200), 0xFFFFFFFF) < 0)
        {
        ...
        if (fill_data_pattern(u32Addr, u32Addr + (0x200), u32Pattern) < 0)
        {
        ...
        if (verify_data(u32Addr, u32Addr + (0x200), u32Pattern) < 0)
        {
            printf("\nData verify failed!\n ");
            return -1;
        }
...
int main()
{
    ...
    if (set_data_flash_base(0x5000) < 0)
    {
    ...
    if (flash_test(0x00100000UL, LDROM_TEST_END, 0x5A5A5A5A) < 0)
    {
        printf("\n\nLDROM test failed!\n");
        goto lexit;
    }
    ...
    if (flash_test(0x3000, 0x5000, 0x5A5A5A5A) < 0)
    {
        printf("\n\nAPROM test failed!\n");
        goto lexit;
    }
    ...
    printf("\n\nData Flash test =>\n");
    if (flash_test(0x5000, 0x6000, 0x5A5A5A5A) < 0)
    {
