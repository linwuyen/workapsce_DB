...
void ADC_FunctionTest()
{
    ...
    {
        printf("\n\nSelect input mode:\n");
        printf("  [1] Single end input (channel 2, 3, 4 and 5)\n");
        printf("  [2] Differential input (input channel pair 1 and 2)\n");
        printf("  Other keys: exit single-cycle scan mode test\n");
        u8Option = getchar();
        if(u8Option == '1')
        {
            /* Set input mode as single-end, Single-cycle scan mode, and select channel 2~5 */
            ADC_Open(ADC, ADC_ADCR_DIFFEN_SINGLE_END, ADC_ADCR_ADMD_SINGLE_CYCLE, BIT2|BIT3|BIT4|BIT5);
        ...
        {
            /* Set input mode as differential, Single-cycle scan mode, and select channel 2 and 4 */
            ADC_Open(ADC, ADC_ADCR_DIFFEN_DIFFERENTIAL, ADC_ADCR_ADMD_SINGLE_CYCLE, BIT2|BIT4);
            ...
            /* Get the conversion result of the sample module */
            for(u32ChannelCount = 2; u32ChannelCount <= 5; u32ChannelCount += 2)
                i32ConversionData[u32ChannelCount] = ADC_GET_CONVERSION_DATA(ADC, u32ChannelCount);
