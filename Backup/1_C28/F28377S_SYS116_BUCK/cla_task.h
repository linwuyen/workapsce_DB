...
#define FG_GETEL(x)   FG_GET(x, sCLA.ELfgStatus)
#define FG_SETEL(x)   FG_SET(x, sCLA.ELfgStatus)
#define FG_RSTEL(x)   FG_RST(x, sCLA.ELfgStatus)

#define FG_ANDELERR(x)   FG_AND(x, sCLA.ELfgError)
#define FG_GETELERR(x)   FG_GET(x, sCLA.ELfgError)
#define FG_SETELERR(x)   FG_SET(x, sCLA.ELfgError)
#define FG_RSTELERR(x)   FG_RST(x, sCLA.ELfgError)
...
/*E-Load Control and Error State*/
enum {
    _ELSTAT_EL_READY    = (0x0000),
    _ELSTAT_ENABLE_EL   = (0x0001<<0),
    _ELSTAT_EL_LDON     = (0x0001<<1),
    _ELSTAT_EL_STATUS   = (_ELSTAT_ENABLE_EL|_ELSTAT_EL_LDON),
};

typedef int16_t FG_ELSTAT;

enum {
    _NO_EL_ERROR        = (0x0000),
    _ERR_EL_OCP         = (0x0001<<0),
    _ERR_EL_OTP         = (0x0001<<1),
    _ALL_EL_ERROR       = (_ERR_EL_OCP|_ERR_EL_OTP),
    _WARNING_EL_UnReg   = (0x0001<<8),
    _ALL_EL_WARNING     = (_WARNING_EL_UnReg),
};

typedef uint16_t FG_ELERR;
/*E-Load Control and Error State*/
