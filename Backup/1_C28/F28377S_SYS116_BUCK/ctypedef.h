...
typedef struct {
    union
    {
        ...
        struct
        {
            uint32_t Data:16;           // Bit 15-0: DAC Data
            uint32_t Address:2;         // Bit 17-16: DAC Channel
            uint32_t Reserved:14;       // Bit 31-18: Reserved
        }Bits;
        struct
        {
            uint32_t LOData:9;          // Bit 8-0: DAC LoData
            uint32_t HIData:9;          // Bit 17-9: DAC (Channel + HiData)
            uint32_t Reserved:14;       // Bit 31-18: Reserved
        }FiFo;
    }SPI_FORMAT;
    
    uint16_t u16Dac_Chl_Chk; 
    uint16_t u16Dac_Data_ACh_Chk;
    uint16_t u16Dac_Data_BCh_Chk;
    uint16_t u16Dac_FgnLDAC;
    uint16_t u16Dac_PutoCnt;
    uint16_t u16Dac_Data_ALLCh_Chk; 
