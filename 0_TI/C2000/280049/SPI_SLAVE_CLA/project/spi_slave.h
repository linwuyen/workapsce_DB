...
{
    SPI_writeDataBlockingFIFO(SPIA_SLAVE_BASE, u16Data);
}

static inline uint16_t rdSpiData(void)
{
    return SPI_readDataNonBlocking(SPIA_SLAVE_BASE);
}

static inline uint16_t isRxEmpty(void)
{
    return (SPI_FIFO_RXEMPTY != SPI_getRxFIFOStatus(SPIA_SLAVE_BASE));
}
...
{
    SPI_resetTxFIFO(SPIA_SLAVE_BASE);
}
