...
int16_t pushModbusByte(uint8_t u8Data, SCI_MODBUS *mbus)
{
    if(0 != mbus->RxBuf[0]) {
        mbus->TxBuf[mbus->pushcnts++] = u8Data;
        return ucMBCRC16((uint16_t)u8Data, &mbus->info.tbcrc);
    }
    else {
        //Broadcast
        return 0;
